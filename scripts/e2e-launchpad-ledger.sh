...
curl -fsS -X POST "$API/api/v1/tokens/$TOKEN_ID/buy" \
  -H 'Content-Type: application/json' \
  -d "{\"user_id\":\"$TEST_USER_ID\",\"amount_in_minor\":100000000,\"client_id\":\"buy-$RUN_ID\"}" \
  >"$ARTIFACT_DIR/buy-result.json"

curl -fsS "$WALLET/v1/accounts/$TEST_USER_ID/transactions?limit=10" >"$ARTIFACT_DIR/wallet-transactions.json"
