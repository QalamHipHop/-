...
۶. wallet-service نیز در credit/debit/internal ledger/transfer/withdraw هنوز چند JSON `int64` داشت. همهٔ این requestها اکنون amount را به‌صورت string می‌گیرند و پیش از ledger call با `parsePositiveInt64` اعتبارسنجی می‌کنند؛ در نتیجه precision JavaScript/JSON در mutationهای باقی‌مانده از بین نمی‌رود. `go test ./...` wallet-service موفق شد و scan نهایی هیچ `Amount int64` در HTTP routes نشان نداد.

۷. ممیزی end-to-end نشان داد فرم ساخت توکن frontend فقط name/symbol/description/social/initialBuy را می‌فرستاد، درحالی‌که CreateToken DTO به chain، contract identifier، total supply، decimals، curve model و graduation threshold نیاز دارد؛ بنابراین مسیر واقعی ساخت توکن از UI ناقص بود. فرم اکنون همهٔ ورودی‌های واقعی را می‌گیرد، `initialBuy` بدون backend حذف شده، chain روی `rial` است و مقادیر بزرگ با string/BigInt validation کنترل می‌شوند. هم‌زمان responseهای HTTP launchpad برای تمام monetary int64ها با `json:",string"` ایمن شدند. frontend و launchpad tests/build موفق شدند.

۸. ممیزی مرز اعتماد بین‌سرویسی نشان داد wallet HTTP فقط یک shared internal token داشت؛ در نتیجه هر سرویس دارای آن credential، از نظر مسیر API می‌توانست به عملیات سطح بالاتر نزدیک شود. اکنون wallet config سه token مجزا برای `backend`، `payment` و `launchpad` دارد؛ production middleware وجود `X-Rial-Service` و token متناظر را الزام می‌کند و legacy fallback فقط خارج production معتبر است. compose، env example و production validator هم‌راستا شدند و tokenها باید distinct باشند. تمام callerها header scope می‌فرستند. هم‌زمان launchpad wallet client از JSON number به string amount منتقل شد تا با wallet API جدید سازگار و precision-safe باشد.

۹. برای جلوگیری از regression در service-scoped authentication، تست‌های HTTP اضافه شد: production بدون `X-Rial-Service` رد می‌شود، scope ناشناخته رد می‌شود، token سرویس دیگر رد می‌شود، token صحیح پذیرفته می‌شود و legacy fallback فقط در development پذیرفته است. کل تست‌های wallet-service موفق شدند.
