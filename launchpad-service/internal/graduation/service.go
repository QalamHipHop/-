...
func NewService(cfg *config.Config, c *curve.Engine, pg *store.PG, rd *store.RD, nc *event.Nats, log *zap.Logger) *Service {
	return &Service{cfg: cfg, pg: pg, rd: rd, nc: nc, curve: c, log: log, notifyCh: make(chan uuid.UUID, 256)}
}
...
func (s *Service) tick(ctx context.Context) {
	...
	if err != nil { s.log.Warn("graduation query", zap.Error(err)); return }
	defer rows.Close()
	var ids []uuid.UUID
	for rows.Next() { var id uuid.UUID; if err := rows.Scan(&id); err == nil { ids = append(ids, id) } }
	for _, id := range ids {
...
func (s *Service) graduate(ctx context.Context, id uuid.UUID) {
	s.mu.Lock()
	defer s.mu.Unlock()
	// run in tx; ensure we don't double-graduate
	err := s.pg.WithTx(ctx, func(tx pgx.Tx) error {
		var graduated bool
		if err := tx.QueryRow(ctx, `SELECT graduated FROM launchpad.tokens WHERE id=$1 FOR UPDATE`, id).Scan(&graduated); err != nil { return err }
		if graduated { return nil }
	...
	if err != nil { s.log.Warn("graduate tx", zap.String("id", id.String()), zap.Error(err)); return }

	// publish graduation event — the AMM adapter / matching engine will create the pool
	if s.nc != nil {
		s.nc.Publish(ctx, "launchpad.graduated", map[string]any{
			"token_id": id.String(),
			"adapter":  s.cfg.Launchpad.AMMAdapter,
			"base":     s.cfg.Launchpad.AMMBase,
		})
	}
	s.log.Info("token graduated", zap.String("id", id.String()))
}
